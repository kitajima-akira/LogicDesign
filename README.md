...
LogicDesignの置き場所はC:\ユーザー\\*ユーザー名*\source\reposにしてください。
他のフォルダにすると問題が起こる場合があります。

エクスプローラーで C:\ユーザー\\*ユーザー名*\source\repos を開いて、アドレスバーにpowershellと入力することで、Powershellのターミナルが開きます。
...
### OSS CAD Suiteのインストール

[公式サイトのダウンロードページ](https://github.com/YosysHQ/oss-cad-suite-build/releases/latest)からWindows用のインストーラー(oss-cad-suite-windows-x64-*日付*.exe)をダウンロードし、次の手順でインストールします。

1. ダウンロードしたインストーラーを実行します。
「WindowsによってPCが保護されました」の表示が出た場合、「詳細情報」 > 右下の「実行」
2. 作成された「oss-cad-suite」フォルダを自分で分かる場所(例. C:\ユーザー\\*ユーザー名*\source\repos)に移動します。ここで、移動先(oss-cad-suiteの場所)のパス(フォルダ名)には空白や日本語の文字を含めないようにしてください。そうでないと、正しく実行できません。
3. インストールの確認として、移動後に、oss-cad-suite内のstart.batファイルを実行してください。ターミナルが開き、次のようにコマンドのプロンプトだけが表示されればOKです。

    [OSS CAD SUITE] *oss-cad-suiteのパス*>

一方、上記のプロンプトの前に次のようなエラーが表示された場合、oss-cad-suiteの置き場所に問題があります。適切な場所にoss-cad-suiteを移動させてください。

    'gdk-pixbuf-query-loaders.exe' は、内部コマンドまたは外部コマンド、
    操作可能なプログラムまたはバッチ ファイルとして認識されていません。 

#### Visual Studio CodeでのOSS CAD Suiteの利用

Visual Studio CodeからOSS CAD Suiteを利用するために、Visual Studio Codeを起動するときには、OSS CAD Suiteのstart.batを起動して表示されるターミナルに「code」と入力して起動する必要があります。次の設定を行うと、タスクバーのアイコンをクリックするだけでVisual Studio Codeを起動できます。
...
3. 作成されたショートカットを1度クリックしてから右クリック > プロパティ
...
8. LogicDesignのフォルダが指定されたフォルダ(C:\ユーザー\\*ユーザー名*\source\repos)ではない場合は、start_code.batを編集し、2行目のフォルダを自分のLogicDesignのフォルダの場所に合わせて書き換える。

設定ができたら、確認のためタスクバーのアイコンをクリックします。
ターミナルが開き、Visual Studio Codeが起動してLogicDesignのフォルダが開いた状態になれば正しく設定できています。

### Visual Studio Codeの設定

#### YoWASPのインストール

Pythonがインストールされていない場合は、[公式サイト](https://www.python.org/)から最新版のstandalone installerをダウンロードしてインストールしてください。具体的には、インストーラーを実行し、最初のウィンドウの下の方にある二つのチェックボックスに両方ともチェックを入れてから「Install Now」を押します。

インストールが完了したら、新しくターミナルを開いて次のコマンドを実行し、バージョン番号が表示されることを確認してください。

    python --version

Pythonがインストールできたら、ターミナルで次のコマンドを実行してください。
...
1. 「Linter settings」をクリックし、現れたリストの「Icarus linter」をクリックしてArgumentsの部分に「-g 2012 -Y .sv -y . -i -o NUL」と入力します。
...
##### フォントをインストールして使用する設定
よりよいフォントを見つけて設定することもできます。

[Visual Studio Codeで使用するフォントを自分でインストールし使用するには](doc/additional_font.md)
