...
2. 作成された「oss-cad-suite」フォルダを自分で分かる場所(例. C:\ユーザー\\*ユーザー名*\source\repos)に移動します。
...
Pythonがインストールされていない場合は、[公式サイト](https://www.python.org/)から最新版をダウンロードしてインストールしてください。具体的には、インストーラーを実行し、最初のウィンドウの下の方にある二つのチェックボックスに両方ともチェックを入れてから「Install Now」を押します。
...
1. 左側の「Icarus linter」をクリックしてArgumentsの部分に「-g 2012 -Y .sv -y . -i -o NUL」と入力します。
